...
function getFallbackRecommendations(listings: any[], budget: number, preferredArea: string, lifestyle: string) {
  ...
  if (searchArea) {
    matchingListings = listings.filter((l) => {
      ...
      if (loc.includes(searchArea)) return true;
      // Common state lookups
      if (searchArea === "ny" && loc.includes("new york")) return true;
      if (searchArea === "ca" && loc.includes("california")) return true;
      if (searchArea === "il" && loc.includes("illinois")) return true;
      if (searchArea === "wa" && loc.includes("washington")) return true;
      return false;
    });
  }
